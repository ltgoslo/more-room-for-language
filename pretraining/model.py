...
class EncoderDecoderModel(nn.Module):
    def __init__(self, config, dummy=False):
        ...
        self.transformer = Decoder(config, dummy=dummy)
        self.classifier = MaskClassifier(config, self.embedding.word_embedding.weight)
        self.paraphrase_encoder = Encoder(config)

        if dummy:
            self.requires_grad_(False)
            for layer in self.transformer.layers:
                layer.cross_attention.amputed_linear.requires_grad_(True)

    def encode_paraphrase(self, x, x_mask):
        static_embeddings, relative_embedding = self.embedding(x)
        contextualized_embeddings = self.paraphrase_encoder(static_embeddings, x_mask.unsqueeze(1).unsqueeze(2), relative_embedding)
        return contextualized_embeddings

    def encode_input(self, x, x_mask, context, context_mask):
        static_embeddings, relative_embedding = self.embedding(x)
        contextualized_embeddings = self.transformer(static_embeddings, x_mask.unsqueeze(1).unsqueeze(2), context, context_mask.unsqueeze(1).unsqueeze(2), relative_embedding)
        return contextualized_embeddings

    def forward(self, x, x_mask, paraphrase, paraphrase_mask, masked_lm_labels=None):
        context = self.encode_paraphrase(paraphrase, paraphrase_mask)
        contextualized_embeddings = self.encode_input(x, x_mask, context, paraphrase_mask)
...
class EncoderModel(nn.Module):
    ...
    def forward(self, x, x_mask, paraphrase, paraphrase_mask, masked_lm_labels=None):
        contextualized_embeddings = self.encode_input(x, x_mask)
        subword_prediction = self.classifier(contextualized_embeddings, masked_lm_labels)
...
class Decoder(nn.Module):
    def __init__(self, config, dummy=False):
        super().__init__()
        self.layers = nn.ModuleList([DecoderLayer(config, dummy) for _ in range(config.num_hidden_layers)])

        for i, layer in enumerate(self.layers):
            layer.mlp.mlp[1].weight.data *= math.sqrt(1.0 / (2.0 * (1 + i)))
            layer.mlp.mlp[-2].weight.data *= math.sqrt(1.0 / (2.0 * (1 + i)))

    def forward(self, x, x_mask, context, context_mask, relative_embedding):
        for layer in self.layers:
            x = layer(x, x_mask, context, context_mask, relative_embedding)
        return x
...
class DecoderLayer(nn.Module):
    def __init__(self, config, dummy=False):
        super().__init__()
        self.attention = SelfAttention(config)
        self.cross_attention = DummyCrossAttention(config) if dummy else CrossAttention(config)
        self.mlp = FeedForward(config)

    def forward(self, x, x_mask, context, context_mask, relative_embedding):
        x = x + self.attention(x, x_mask, relative_embedding)
        x = x + self.cross_attention(x, context, context_mask, relative_embedding)
        x = x + self.mlp(x)
        return x
