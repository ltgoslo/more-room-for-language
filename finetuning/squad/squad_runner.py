...
from modeling_ltgbert import LtgBertForQuestionAnswering
import numpy as np
...
squad_v2 = False
#model_checkpoint = 'davda54/wiki-no-retrieval-patch-base'
...
def convert_json_to_hf(data):
    ...
    for question, context in tqdm(q_c_pairs):
        dataset = dataset.add_item({
            "id": str(question["id"]),
            "title": str(question["id"]),
            "context": context,
...
for seed in [1234, 2345, 3456]:
    print(f'Starting loop for seed={seed}..')
    model = LtgBertForQuestionAnswering.from_pretrained(model_checkpoint, trust_remote_code=True)
...
print(r)
with open(f"./camera_ready_results/results_{model_checkpoint.split('/')[-1]}.txt", 'a') as f:
    f.write(' & '.join([model_checkpoint.split('/')[-1]] + list(r.values())) + '\n')
    o = " ".join([f"{key}: {value}" for key, value in results.items()])
    f.write("\n")
    f.write(o)
