...
class ModelArguments:
    ...
    classifier_dropout: float = field(
            default=0.2,
            metadata={"help": "Classifier dropout."}
    )
    weight_layer_only: bool = field(
            default=False,
            metadata={"help": "Only prev_layer_weights and classifier are fine-tuned"}
    )
...
def main():
    ...
    if data_args.task_name is not None:
        ...
        if training_args.do_predict:
            ...
            print(training_args.do_predict, flush=True)
    ...
    is_mrpc_or_qqp = task_name == "mrpc" or task_name == "qqp"
    is_cola = task_name == "cola"
    print(f"Is QQP or MRPC: {is_mrpc_or_qqp}", flush=True)
    training_args.task_name = task_name
    ...
    try:
        ...
        if isinstance(config, T5Config):
            ...
            raise e
    if model_args.weight_layer_only:
        for name, param in model.named_parameters():
            if ("head" not in name and not name.startswith("score")) and "prev_layer_weights" not in name: # classifier layer
    ...
    # Padding strategy
    if False and data_args.pad_to_max_length:
        padding = "max_length"
    else:
    ...
    if training_args.do_train:
        ...
        if training_args.do_eval:
            joint_dataset = raw_datasets["train"]
            train_dataset, eval_dataset = train_test_split(joint_dataset, test_size=0.1, random_state=training_args.seed)
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if data_args.task_name is not None:
            ...
            return {"mse": ((preds - p.label_ids) ** 2).mean().item(),
                    "pearson_r": pearsonr(p.label_ids, preds)[0],
                    "spearman_r": spearmanr(p.label_ids, preds)[0]}
        elif is_binary:
            return {"accuracy": (preds == p.label_ids).astype(np.float32).mean().item(),
                    "f1": f1_score(y_true=p.label_ids, y_pred=preds, average="binary"),
                    "f1_0": f1_score(y_true=p.label_ids, y_pred=preds, pos_label=0, average="binary"),
    ...
    if is_regression:
        training_args.metric_for_best_model = "pearson_r"
    elif is_mrpc_or_qqp:
    ...
    training_args.logging_strategy = training_args.evaluation_strategy
    training_args.logging_steps = data_args.eval_every
    training_args.saving_steps = data_args.eval_every
    training_args.max_grad_norm = 1_000_000
    ...
    if training_args.do_eval:
        ...
        for eval_dataset, task in zip(eval_datasets, tasks):
            ...
            wandb.log({f"eval_final/{k}": v for k, v in metrics.items()})
